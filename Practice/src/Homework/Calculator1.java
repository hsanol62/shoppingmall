...
public  class Calculator1 extends JFrame{
	...
	public Calculator1() {
		...
		for (int rows = 0; rows<5; rows++) {
			for(int cols = 0; cols <5; cols++ ) {
				...
				panel.add(buttons[index]); //ActionListener 정의
				index++;
			}
		}
		Component dispaly = null; //오류나서 추가
	...
	public void actionPerformed(ActionEvent e) { //didn't work, changed name,add 1
		...
		if(command.charAt(0) == 'C') { //command.charAt?
			startOfNumber = true;
					result = 0;
					operator = "="; //Syntax error on token "operator" AssignmentOperator expected after this token = '=' 추가
					display.setText("0.0");
